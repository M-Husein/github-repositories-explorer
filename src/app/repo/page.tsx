...
type pageProps = {
  ...
  searchParams?: { user?: string, reponame?: string }
}

// export async function generateMetadata(
//   { searchParams }: pageProps,
// ): Promise<Metadata> {
//   const title = [searchParams?.user, searchParams?.reponame].filter(Boolean).join("/");
//   return {
//     title,
//     openGraph: {
...
export default function Detail({ searchParams }: pageProps){
  return (
    <div className="w-full max-w-5xl mx-auto p-4 flex flex-col">
      ...
      {[searchParams?.user, searchParams?.reponame].filter(Boolean).join("/")}
    </div>
  );
}
